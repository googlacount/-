...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col pb-20 bg-slate-50">
      <header className="sticky top-0 z-50 glass px-6 py-4 flex items-center justify-between border-b shadow-sm">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-indigo-600 rounded-xl flex items-center justify-center text-white shadow-lg">
            <i className="fas fa-graduation-cap text-lg"></i>
          </div>
          <div className="hidden sm:block">
            <h1 className="font-bold text-slate-800 leading-none">Pro Quiz Architect</h1>
            <p className="text-[10px] text-slate-400 uppercase tracking-widest mt-1">{t.dashboard}</p>
          </div>
        </div>

        <div className="flex items-center gap-2">
          <button onClick={saveToLibrary} title={t.saveToLibrary} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all">
            <i className="fas fa-save text-xl"></i>
          </button>
          <button onClick={handlePreview} disabled={questions.length === 0} className="bg-slate-800 hover:bg-slate-900 text-white px-4 py-2 rounded-lg font-medium text-sm disabled:opacity-50">
            {t.preview}
          </button>
          <button onClick={handleDownload} disabled={questions.length === 0} className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-lg font-bold text-sm shadow-indigo-200 shadow-lg disabled:opacity-50 transition-all active:scale-95">
            {t.download}
          </button>
        </div>
      </header>
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-6 py-8">
        <div className="flex flex-wrap gap-1 bg-slate-200/50 p-1 rounded-xl mb-8 w-fit mx-auto shadow-inner">
          <button onClick={() => setActiveTab('editor')} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2 ${activeTab === 'editor' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500'}`}>
            <i className="fas fa-pencil-alt"></i> {t.builder}
          </button>
          <button onClick={() => setActiveTab('bank')} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2 ${activeTab === 'bank' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500'}`}>
            <i className="fas fa-database"></i> {t.bank}
          </button>
          <button onClick={() => setActiveTab('library')} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2 ${activeTab === 'library' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500'}`}>
            <i className="fas fa-book"></i> {t.library}
          </button>
          <button onClick={() => setActiveTab('settings')} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2 ${activeTab === 'settings' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500'}`}>
            <i className="fas fa-sliders-h"></i> {t.settings}
          </button>
          <button onClick={() => setActiveTab('cloud')} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2 ${activeTab === 'cloud' ? 'bg-white text-sky-600 shadow-sm' : 'text-slate-500'}`}>
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-sm border-t py-4 px-6 flex justify-between items-center text-[10px] text-slate-400">
